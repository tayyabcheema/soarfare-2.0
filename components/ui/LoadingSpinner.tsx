...
export const ModernLoader: React.FC<{ size?: "sm" | "md" | "lg" | "xl" }> = ({
  size = "lg",
}) => {
  const sizeMap = {
    sm: "w-8 h-8",
  ...
  return (
    <div className={`${sizeMap[size]} relative`}>
      ...
      <div
        ...
        style={{
          background:
            "conic-gradient(from 0deg, #FD7300, #F27709, #FD7300, transparent)",
...
export const PageLoader: React.FC<{
  message?: string;
  showOverlay?: boolean;
}> = ({ message = "Loading...", showOverlay = false }) => {
  if (showOverlay) {
    return (
      <div className="w-full fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center">
        <div className="flex justify-center items-center bg-white rounded-xl p-8 max-w-sm w-full mx-4 animate-fade-in">
          <div className="text-center">
