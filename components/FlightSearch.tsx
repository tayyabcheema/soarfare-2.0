...
const highlightMatch = (text: string, query: string) => {
  ...
  return text.split(regex).map((part, i) =>
    regex.test(part) ? (
      <span key={i} className="font-bold text-orange-500">
        {part}
      </span>
    ) : (
      part
...
const FlightSearch: React.FC = () => {
  ...
  const [showMultiCityDropdown, setShowMultiCityDropdown] = useState<{
    [key: string]: boolean;
  }>({});
  const [showMultiCityDatePicker, setShowMultiCityDatePicker] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  //Multi select Line button handle

  const [isLineLengthOne, setIslineLengthOne] = useState(true);
  ...
  const handleAirportSelect = (
    ...
    if (segmentIndex !== undefined) {
      ...
      setShowMultiCityDropdown((prev) => ({
        ...prev,
        [`${field}-${segmentIndex}`]: false,
      }));
    } else {
      // Main form
      if (field === "from") {
  ...
  useEffect(() => {
    setIslineLengthOne(multiCitySegments.length === 0);
  }, [multiCitySegments]);
    ...
    try {
      ...
      if (tripType === "single") {
        ...
        multiCitySegments.forEach((segment, index) => {
          searchParams.append(
            `from_mc_${index}`,
            extractAirportCode(segment.from)
          );
  ...
  return (
    <div className="relative">
      ...
      {(showFromDropdown ||
        ...
        showPassengerPanel ||
        Object.values(showMultiCityDropdown).some(Boolean) ||
        Object.values(showMultiCityDatePicker).some(Boolean)) && (
        <div className="fixed inset-0 z-30" onClick={closeAllModals} />
      )}
      ...
      >
          ...
          {[
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setTripType(e.target.value as "round" | "single" | "multi")
                }
        ...
        {/* Main Form Fields */}
        <div
          className={`grid gap-4 lg:gap-6 items-end ${
            isMobile
              ? "grid-cols-2"
              : tripType === "round"
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="bg-transparent border-b-2 border-white/30 pb-2 min-h-[40px] flex items-center justify-between">
              ...
              />
              <svg
                className="w-4 h-4 text-[#FD7300] ml-2"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
            ...
            {showFromDropdown &&
              fromQuery.length >= 2 &&
              filterAirports(fromQuery).length > 0 && (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="font-medium text-gray-900">
                            ...
                            <span>
                              {highlightMatch(airport.iata, fromQuery)}
                            </span>
                            )
                          </div>
                          <div className="text-sm text-gray-500">
                            {highlightMatch(airport.city, fromQuery)},{" "}
                            {highlightMatch(airport.country, fromQuery)}
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          <div className="relative">
            ...
            {showToDropdown &&
              toQuery.length >= 2 &&
              filterAirports(toQuery).length > 0 && (
                <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-200 w-full max-w-sm z-50">
                  <div className="max-h-60 overflow-y-auto">
                    {filterAirports(toQuery).map((airport) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-500">
                            {highlightMatch(airport.city, toQuery)},{" "}
                            {highlightMatch(airport.country, toQuery)}
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          {/* Search Button and Add more button*/}
          <div className="w-full flex justify-end items-center gap-5">
            {tripType !== "multi" ? (
              <>
                <div
                  className={`flex justify-center ${
                    isMobile ? "col-span-1 mt-0" : ""
                  }`}
                >
                  <button
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      <svg
                        className="w-5 h-5 md:w-6 md:h-6 text-[#FD7300]"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                      ...
                      {multiCitySegments.length < 3 && (
                        <div className="flex justify-center">
                          <button
                            ...
                            className="flex items-center space-x-2 px-3 py-3 bg-white text-[#0C2442] rounded-lg hover:bg-gray-100 transition-colors font-medium max-sm:text-[14px] max-sm:px-2 max-sm:py-3 max-sm:space-x-1"
                          >
                            <span>+</span>
                            <span>Add More</span>
                          </button>
            ...
            {multiCitySegments.map((segment, index) => (
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="bg-transparent border-b-2 border-white/30 pb-2 min-h-[40px] flex items-center justify-between">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setMultiCitySegments((prev) =>
                          prev.map((seg, i) =>
                            i === index ? { ...seg, fromQuery: value } : seg
                          )
                        );
                      }}
                      onFocus={() =>
                  ...
                  {showMultiCityDropdown[`from-${index}`] &&
                    filterAirports(segment.fromQuery || "").length > 0 && (
                      <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-200 w-full max-w-sm z-50">
                        <div className="max-h-60 overflow-y-auto">
                          {filterAirports(segment.fromQuery || "").map(
                            (airport) => (
                              <div
                                key={airport.iata}
                                onMouseDown={() => {
                ...
                <div className="relative">
                  ...
                  {showMultiCityDropdown[`to-${index}`] &&
                    filterAirports(segment.toQuery || "").length > 0 && (
                      <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-200 w-full max-w-sm z-50">
                        <div className="max-h-60 overflow-y-auto">
                          {filterAirports(segment.toQuery || "").map(
                            (airport) => (
                              <div
                                key={airport.iata}
                                onMouseDown={() => {
                ...
                <div className="relative">
                  ...
                  <div
                    ...
                    onClick={() => {
                      closeAllModals();
                      setShowMultiCityDatePicker((prev) => ({
                        ...prev,
                        [`date-${index}`]: true,
                      }));
                    }}
        ...
        {Object.keys(errors).length > 0 && (
          <div className="mt-6 p-4 bg-red-100 border border-red-400 rounded-lg">
            <div className="text-red-700">
              ...
              {errors.returnBeforeTravel && (
                <div>• {errors.returnBeforeTravel}</div>
              )}
              {errors.multiCity &&
                errors.multiCity.map((error, index) => (
